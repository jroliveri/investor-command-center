...
- Local RSI, MACD, and volume indicator cache calculated from Yahoo Finance historical OHLCV data
...
- SQLite migrations for `accounts`, `holdings`, `transactions`, `dividends`, `goals`, `watchlists`, `watchlist`, `import_batches`, `portfolio_snapshots`, `dashboard_widgets`, `market_quote_cache`, `market_price_history`, and `technical_indicator_cache`
...
Research quote, historical OHLCV, and technical indicator cache records are stored locally in SQLite and are not brokerage-verified records. They should not be treated as authoritative account data.
...
- RSI, MACD, and volume displays are first-pass context only and do not create recommendations, alerts, trades, or portfolio updates.
